...
class Evaluator:
    ...
    def analyze_results(self, y_test, y_preds, history, 
                        online_training_acc, online_training_losses):
        # Confusion Matrix
        precision, recall, accuracy, f1 = \
            self.confusion_matrix(y_preds, y_test)
        ...
        self.plot_online_training_curve(online_training_acc, 
            ...
            def __init__(self):
                ...
                self.signal_dict = \
                    dict((pd.Timestamp(date).to_pydatetime().date(), signal)
                                        for date, signal in zip(pred_trade_signals['Date'], 
